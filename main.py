...
def run_route(node: Node, parent_node: Node, content_file: ContentFile, time: int):
    ...
    if content_file.is_ressource_in_stock(node.process):
        content_file.run_process(node.process, parent_node.process if parent_node else None, time)
...
def new_run_route(node: Node, parent_node: Node, content_file: ContentFile, route_stock_requirements: RouteStockRequirements, multiplicator, time):
    # si process déja lancer alors return
    if _ := next((running for running in content_file.running_process_list if running.process.name == node.process.name), None):
        return
    ...
    for need in node.process.needs:
        ...
        # if check si il y a CE need en attente de process
        elif _ := next((running for running in content_file.running_process_list if running.process_from.name == node.process.name and running.process.create_item(need)), None):
            continue
        else:
            
            need_stock = next((stock for stock in content_file.stock_list if stock.name == need.name), None)
            need_stock = need_stock.quantity if need_stock else 0
            
            
            running_process = [running for running in content_file.running_process_list if running.process.create_item(need)]
            
            need_to_create = sum([next(result for result in process.process.results if result.name == need.name).quantity for process in running_process]) if len(running_process) else 0
            
            total_need_other_process_need = sum([next((result for result in item.process.results if result.name == need.name)).quantity * item.multiplicator for item in route_stock_requirements.road_map if item.process.name != node.process and item.process.create_item(need)])
            
            number_need_rest = need_stock + need_to_create - total_need_other_process_need
            ...
            elif ressource_process := next((process for process in content_file.get_ressources_process() if process.create_item(need)), None):
                result = next((result for result in ressource_process.results if result.name == need.name))
                for _ in range(math.ceil((number_need_need_for_that_process - number_need_rest) / result.quantity)):
                    content_file.run_process(ressource_process, node.process, time)
            # if check si CE need viens d'un child
            elif child := next((child for child in node.children if child.process.create_item(need)), None):
                # child.process.display()
                result = next((result for result in child.process.results if result.name == need.name))
                new_run_route(child, node, content_file, route_stock_requirements, multiplicator * math.ceil(need.quantity / result.quantity), time)
        
        
def calculate_stock_route(node: Node, route_stock_requirements: RouteStockRequirements, multiplicator = 1, depth = 1):
    node.display()
    
    route_stock_requirements.add_road_map_item(node.process, multiplicator)
    
    results = [result for result in node.process.results if result.name not in [need.name for need in node.process.needs]]
    
    for result in results:
        route_stock_requirements.add_require_stock(Item(result.name, result.quantity*multiplicator), [Item(need.name, need.quantity*multiplicator) for need in node.process.needs], depth)
    
    needs = [need for need in node.process.needs if need.name not in [result.name for result in node.process.results]]
    
    for need in needs:
        # print(need.name)
        if child := next((child for child in node.children if child.process.create_item(need)), None):
            calculate_stock_route(child, route_stock_requirements, multiplicator * need.quantity, depth+1)
...
def main():
    ...
    for requirement in main_nodes[0].requirements:
        print(requirement.result.name, requirement.result.quantity, "[", " ".join(["|".join([need.name, str(need.quantity)]) for need in requirement.needs]), "]", requirement.depth)
    ...
    while True:
        ...
        if running := next((running for running in content_file.running_process_list if not running.process_from), None):
        ...
        actual_time = min([running.end_time for running in content_file.running_process_list] if (len(content_file.running_process_list) > 0) else ([actual_time+1, actual_time+1]))
